...
def model_press(name, complete_db_name):
    check_existence_folder(f"{data_path}/pHMM_associate_files/pHMM_model/{name}", True)

    # cats all the different pHMM models into a single model.
    os.system(f"cat {data_path}/pHMM_associate_files/pHMM/{name}/* > {data_path}/pHMM_associate_files/pHMM_model/{name}/{complete_db_name}")
    # hmmpress of the full model is needed for the model to work.
    os.system(f"hmmpress {data_path}/pHMM_associate_files/pHMM_model/{name}/{complete_db_name}")

# Defining paths and files
data_path = os.getcwd()

# Checks if a file with the name 'pHMM_model', else creates it
check_existence_folder(f"{data_path}/pHMM_associate_files/pHMM_model", True)
