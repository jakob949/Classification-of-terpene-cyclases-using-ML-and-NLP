...
def aligment_hmmbuild(file_path, name):
    ...
    for file in files:
        ...
        if len(nums_of_new_lines) != 2:
            os.system(
                f"clustalo -i {file_path}/'{file}' > {data_path}/pHMM_associate_files/aligments/{name}/'{file[:-4]}.st' --outfmt=st ")

            # After the MSA is done. The profile HMM is computed
            os.system(
                f"hmmbuild --mx BLOSUM62 {data_path}/pHMM_associate_files/pHMM/{name}/'{file[:-4]}'.hmm {data_path}/pHMM_associate_files/aligments/{name}/'{file[:-4]}.st' ")
        ...
        # An assumption used here is that a single sequence file contains two: '\n'
            ...
            os.system(
                f"hmmbuild --mx BLOSUM62 {data_path}/pHMM_associate_files/pHMM/{name}/'{file[:-4]}'.hmm {file_path}/'{file}' ")


data_path = os.getcwd()

check_existence_folder(f"{data_path}/pHMM_associate_files/aligments/small", True)
check_existence_folder(f"{data_path}/pHMM_associate_files/aligments/large", True)
check_existence_folder(f"{data_path}/pHMM_associate_files/pHMM/small", True)
check_existence_folder(f"{data_path}/pHMM_associate_files/pHMM/large", True)



aligment_hmmbuild(f"{data_path}/pHMM_associate_files/class/small", "small")
aligment_hmmbuild(f"{data_path}/pHMM_associate_files/class/large", "large")
